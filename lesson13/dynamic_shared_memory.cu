...
extern __shared__ float s_array[]; // 定义动态共享内存变量必须使用extern进行修饰
// extern __shared__ float * s_array; 该数组定义方式错误，不能使用指针进行定义
...
int main(int argc, char **argv)
{
    ...
    dim3 block(32);
    dim3 grid(2);
    kernel_1<<<grid, block, 32>>>(d_A, nElems);// <<<>>>内部需要额外指定动态共享内存的大小
