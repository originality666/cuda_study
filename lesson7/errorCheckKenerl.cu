...
int main(void)
{
    ...
    // 5、调用核函数在设备中进行计算,线程数量是不能超过1024的，所以会在核函数中报错
    dim3 block(2048);
    dim3 grid((iElemCount + block.x - 1) / 2048); 

    addFromGPU<<<grid, block>>>(fpDevice_A, fpDevice_B, fpDevice_C, iElemCount);    // 调用核函数
    ErrorCheck(cudaGetLastError(), __FILE__, __LINE__); // 检测核函数错误
    ErrorCheck(cudaDeviceSynchronize(), __FILE__, __LINE__); // 检测核函数错误
